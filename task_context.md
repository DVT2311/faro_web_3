...
## Trạng thái hiện tại
**Session 0 — Scaffold dự án & tooling đã làm xong về mặt kỹ thuật**, đang chờ người dùng xác nhận trước khi hỏi push git và chuyển sang Session 1 (design tokens + font + reset). Toàn bộ 11 session còn lại (1 → 10) trong `PROGRESS.md` vẫn ở trạng thái ⬜ Chưa làm — trang `public/index.html` hiện chưa có bất kỳ section nội dung thật nào, chỉ là khung rỗng.
...
- **Chưa** commit/push gì lên git — theo đúng quy tắc phải hỏi người dùng trước.
...
- Đang chờ người dùng xác nhận kết quả Session 0 và trả lời có push git hay không.

## Bước tiếp theo
1. Chờ người dùng xác nhận Session 0 đạt yêu cầu + quyết định push git hay chưa.
2. Nếu đồng ý: commit + push, cập nhật cột "Đã push git" trong `PROGRESS.md`.
3. Bắt đầu **Session 1 — Design tokens + font + reset**: điền nội dung thật cho 3 file `public/css/base/*.css` theo đặc tả ở `DESIGN.md` (mục "Session 1"), dựng 1 trang test hiển thị chữ mẫu 2 font để đối chiếu Figma.
